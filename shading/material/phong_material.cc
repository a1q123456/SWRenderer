...
glm::vec4 PhongMaterial::GetPixelColor(const ProgramDataPack& args) const noexcept
{
    ...
    auto imgY = std::clamp((int)std::round(uvw.y * textureH), 0, textureH - 1) + 1;
    glm::vec4 outColor{
    ...
    for (auto &&light : lightEntry)
    {
        ...
        auto specular = std::pow(std::clamp(glm::dot(reflectVec, cameraDir), 0.f, 1.f), shininess) * specularStrength;
